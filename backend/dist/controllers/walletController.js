...
//  Set primary wallet
export const setPrimaryWallet = async (req, res) => {
    try {
        ...
        await Wallet.updateMany({ userId: user._id }, { isPrimary: false });
        const wallet = await Wallet.findOneAndUpdate({ _id: walletId, userId: user._id }, { isPrimary: true }, { returnDocument: 'after' });
        if (!wallet) {
