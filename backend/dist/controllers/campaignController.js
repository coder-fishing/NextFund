...
export const createCampaign = async (req, res) => {
    try {
        ...
        const userWallet = await Wallet.findOne({ userId: user._id }).select("_id");
        if (!userWallet) {
            res.status(403).json({ message: "Please connect wallet before creating campaign" });
            return;
        }
...
export const getCampaignByUserEmail = async (req, res) => {
    try {
...
export const getApprovedCampaigns = async (_req, res) => {
    try {
...
export const getCampaignWithStatus = async (req, res) => {
    try {
...
export const updateCampaignStatus = async (req, res) => {
    try {
        ...
        const campaign = await Campaign.findByIdAndUpdate(id, { status }, { returnDocument: 'after' });
        if (!campaign) {
            res.status(404).json({ message: "Campaign not found" });
            return;
        }
