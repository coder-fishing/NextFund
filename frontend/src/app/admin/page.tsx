...
import { updateCampaignStatus as apiUpdateStatus, bulkModerateCampaigns } from '@/services/adminService';
import { useAdminStats, useAdminCampaigns, useAdminUsers, useSystemSettings } from '@/hooks/useAdmin';
...
export default function AdminDashboard() {
  ...
  // Custom Hooks
  const { stats, loading: loadingStats, refetch: refetchStats } = useAdminStats(isAdmin);
  const {
    campaigns,
  ...
  } = useAdminCampaigns(isAdmin && activeTab === 'campaigns');
  ...
  const { isAiEnabled, togglingAi, toggleAiSetting } = useSystemSettings(isAdmin);
  ...
  const handleStatusUpdate = async (id: string, status: 'approved' | 'rejected') => {
    const confirmed = confirm(
      `Bạn có chắc chắn muốn ${status === 'approved' ? 'DUYỆT' : 'TỪ CHỐI'} chiến dịch này?`
    );
    ...
    <div className="min-h-screen bg-gray-50 pb-20">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-gray-500 mt-1 font-medium">
                Quản lý hệ thống, phê duyệt chiến dịch và người dùng.
              </p>
            </div>
            ...
            <div className="flex items-center gap-4">
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2 rounded-2xl bg-indigo-50 text-indigo-600 border border-indigo-100 hover:bg-indigo-100 transition-colors text-xs font-bold ${
              ...
              <div className="flex items-center gap-4 bg-gray-50 px-4 py-2 rounded-2xl border border-gray-100">
                ...
                <button
                  onClick={toggleAiSetting}
                  disabled={isAiEnabled === null || togglingAi}
                  className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none ${
                    isAiEnabled ? 'bg-emerald-500' : 'bg-gray-300'
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
          <StatCard label="Tổng Campaign" value={loadingStats ? '...' : stats?.totalCampaigns ?? 0} />
          <StatCard label="Hoàn thành 100%" value={loadingStats ? '...' : stats?.completedGoalCount ?? 0} />
          <StatCard label="Tổng ETH đạt được" value={loadingStats ? '...' : `${stats?.totalEth ?? 0} ETH`} />
          <StatCard label="% Đạt được TB" value={loadingStats ? '...' : `${stats?.avgGoalPercentage ?? 0}%`} />
        </div>
            ...
            {activeTab === 'campaigns' ? (
              <>
                ...
                <div className="flex flex-wrap gap-2 mb-6">
                  {(['all', 'pending', 'approved', 'rejected', 'manual'] as StatusFilter[]).map((status) => (
                    <button
                      key={status}
                      onClick={() => {
                ...
                <Pagination
                  ...
                  totalPages={campaignPagination?.totalPages ?? 1}
