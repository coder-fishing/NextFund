    ...
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-5">
        <div className="flex items-center justify-between gap-2">
          ...
          <button type="button" onClick={() => onEditStep?.(0)} className="text-xs font-semibold text-slate-500">
            Step 1
          </button>
        </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-5">
        <div className="flex items-center justify-between gap-2">
          ...
          <button type="button" onClick={() => onEditStep?.(2)} className="text-xs font-semibold text-slate-500">
            Step 3
          </button>
        </div>
        ...
        {selectedImagePreviews[0] ? (
          <div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {selectedImagePreviews.map((preview, index) => (
              <Image
                key={`${preview}-${index}`}
                src={preview}
                alt={`Campaign cover preview ${index + 1}`}
                width={900}
                height={360}
          ...
          <p className="mt-3 text-sm text-slate-700">No image selected</p>
        )}
      </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-5">
        <div className="flex items-center justify-between gap-2">
          ...
          <button type="button" onClick={() => onEditStep?.(3)} className="text-xs font-semibold text-slate-500">
            Step 4
          </button>
        </div>
        ...
        <input
          ...
          onChange={(e) => setFormData((prev) => ({ ...prev, title: e.target.value }))}
          placeholder="Donate to help..."
          className="mt-3 w-full rounded-xl border border-slate-300 px-4 py-3 text-sm outline-none focus:border-emerald-500"
        />
      </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-5">
        <div className="flex items-center justify-between gap-2">
          ...
          <button type="button" onClick={() => onEditStep?.(1)} className="text-xs font-semibold text-slate-500">
            Step 2
          </button>
        </div>
        ...
        <div className="mt-3 grid gap-4 sm:grid-cols-2">
          ...
          <div>
            <label className="mb-2 block text-sm font-medium text-slate-700">Goal amount (VND)</label>
            <input
              type="number"
              min={0.0001}
              step="any"
        ...
        <p className="mt-3 text-sm font-medium text-slate-700">Preview amount: {formatCurrency(formData.goalAmount)}</p>
        <p className="mt-1 text-sm text-slate-700">
          Category preview: {categoryLabelMap[formData.category] ?? "General"}
        </p>
      </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-5">
        ...
        <textarea
          ...
          placeholder="Who are you raising funds for? Why is this cause important? How will the funds be used?"
          className="mt-3 w-full resize-none rounded-xl border border-slate-300 px-4 py-3 text-sm outline-none focus:border-emerald-500"
        />
      </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-5">
        <div className="flex items-center justify-between gap-2">
          ...
          <button type="button" onClick={() => onEditStep?.(4)} className="text-xs font-semibold text-slate-500">
            Step 5
          </button>
        </div>
        ...
        <SecondaryButton
          ...
          onClick={connectWallet}
          disabled={connectingWallet}
          className="mt-3 w-full rounded-xl border border-emerald-600 text-emerald-700"
        >
          {connectingWallet ? "Connecting..." : "Connect Wallet"}
        </SecondaryButton>
      </div>
